...
from experiments.predefined_world.failure_plot import plot_sequential_failure_response
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--no-failure",
        action="store_true",
        help="Disable every configured landing-zone failure.",
    )
    return result
...
def run(arguments: argparse.Namespace) -> Path:
    ...
    failure_schedule = resolve_failure_schedule(
        simulation_config,
        disabled=bool(arguments.no_failure),
    )
    ...
    plot_sequential_failure_response(
        metrics=result.metrics,
        events=result.events,
        controller=controller,
        directory=figures,
